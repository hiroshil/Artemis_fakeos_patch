...
function set_eval(text)
	...
	elseif data:find("[%+%-%/%*%&%|%!%(%)]") then
		...
		e:tag{"var", name="t.tmpex", data=('$'..data), writelocal="1"}
		data = e:var("t.tmpex")
	elseif data:find("^[fgs]%.[a-z]") then
		data = get_eval(data)
	end

	-- 数値変換
	if type(data) == "string" and data:find("^[0-9]+$")then data = tn(data) end
	...
	if hd == 'f' then
		if nm == "_esys" then
			...
			if init.game_varzerodel == "on" and data == 0 then data = nil end	-- メモリ削減
			message("通知", hd, '.', nm, '=', data or "0 [del]")
			scr.vari[nm] = data
			flg.eval = true
		end
	elseif hd == 'g' then
...
function get_eval(text)
	...
	elseif head == 's' then
		...
		elseif name == "sp" and game.sp then ret = 1		-- mobile機なら 1 を返す(android/iOS)

		-- steam
		elseif name == "steam" and init.steam == "on"		then ret = 1	-- steam normal
		elseif name == "deck"  and steam and steam_deckflag then ret = 1	-- steam deck
--		elseif name == "dmmpl" and init.dmmpl_exe			then ret = 1	-- dmm player
--		elseif name == "softdc" and init.softdc_exe			then ret = 1	-- ソフト電池
...
function cond(cond)
	...
	if cond then
		...
		-- Artemisに投げて計算してもらう
		e:tag{"var", name="t.cond", data=('$'..data), writelocal="1"}
		ret = tn(e:var("t.cond"))
